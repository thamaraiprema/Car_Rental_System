...
public class SearchByBrand implements Search{
	...
	public void searchFilter() {
		...
		try {
		    ...
		    PreparedStatement statement = conn.prepareStatement("SELECT COUNT(*) FROM car.CAR WHERE brand=? and status='Available'");
		    ...
		    if (count == 0) {
		        ...
		        PreparedStatement fetchStatement = conn.prepareStatement("SELECT * FROM car.CAR WHERE brand=? and status='Available'");
		        fetchStatement.setString(1, brand);
